...
while ($true) {
    try {
        ...
        $isPXE      = $false
        $clientArch = 0
        $msgType    = 0   # option 53: 1=DISCOVER, 3=REQUEST
        $i = 240
        while ($i -lt ($recv - 1)) {
            $opt = $buf[$i]
            if ($opt -eq 255) { break }
        ...
        if ($msgType -ne 1 -and $msgType -ne 3) { continue }
        ...
        # PXE proxy: reply OFFER to DISCOVER, ACK to REQUEST
        $replyType = if ($msgType -eq 1) { 2 } else { 5 }
        $typeLabel = if ($msgType -eq 1) { "DISCOVER->OFFER" } else { "REQUEST->ACK" }
        Write-Log "PXE $typeLabel from $ep arch=$clientArch ($archLabel) -> $([System.Text.Encoding]::ASCII.GetString($bootFile))"
