...
$bootFile = [System.Text.Encoding]::ASCII.GetBytes('pxelinux.0')
...
Write-Log "Listening on 0.0.0.0:67"
$buf = [byte[]]::new(1500)
...
while ($true) {
    try {
        ...
        Write-Log "PXE DISCOVER from $ep"
        $r = [byte[]]::new(300)
        ...
        [Array]::Copy($bootFile,0,$r,108,$bootFile.Length)
        ...
        Write-Log "Sent proxy DHCP ($sent bytes) to broadcast"
    } catch {
